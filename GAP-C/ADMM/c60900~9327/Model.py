...
class ADMM:
    def __init__(self):
        ...
        self.rpo=0.5 #penalty parameter
        #results
...
#step 1:UB generation
            ...
            repeat_num.append(len(S_2))
            unserved_num.append(len(S_0))
...
#multiplier updation
            ...
            # if i >= 20:
            #     if (repeat_num[i]+unserved_num[i]) ** 2 > 0.25 * (repeat_num[i-1]+unserved_num[i-1]) ** 2:
            #         self.rpo +=1
            #     if (repeat_num[i]+unserved_num[i]) ** 2 == 0:
            #         self.rpo = 1
    ...
    def g_solve_KS_of_LR(self, m):
        ...
        for j in range(self.g_number_of_jobs):
            ...
            self.Knapsack.variables.add(obj=[cost], types=[self.Knapsack.variables.type.binary],
                                        names=["x_{}".format(j)])
        # capacity (resource)
